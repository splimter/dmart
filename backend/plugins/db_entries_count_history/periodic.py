...
async def snapshot(session: AsyncSession) -> None:
    ...
    tables_result = await session.execute(
        text("SELECT table_name FROM information_schema.columns WHERE column_name = 'space_name' AND table_schema = 'public'")
    )
    ...
    for space_name, current_count in spaces_counts.items():
        last_row = (
            await session.execute(
                text("SELECT entries_count FROM count_history WHERE spacename = :sn ORDER BY recorded_at DESC LIMIT 1"),
                {"sn": space_name},
            )
        ).fetchone()
        ...
        if not last_row or last_row[0] != current_count:
            await session.execute(
                text("INSERT INTO count_history (spacename, entries_count) VALUES (:sn, :cnt)"),
